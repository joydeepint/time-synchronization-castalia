...
class TimerService: public virtual cSimpleModule {
 private:
	double timerDrift;                  // Initial clock drift
	ResourceManager *resMgr;            // A pointer to the Resource Manager module
	simtime_t clockLastDriftUpdate;     // Time of last drift update

 protected:
	std::vector<TimerServiceMessage*> timerMessages;

	simtime_t getClock();
	void setClock(simtime_t new_offset);
	...
	simtime_t getOffset();
	void setResourceModule(ResourceManager *);
	simtime_t clockDriftAdjust();
	simtime_t clockDriftCompensation();
