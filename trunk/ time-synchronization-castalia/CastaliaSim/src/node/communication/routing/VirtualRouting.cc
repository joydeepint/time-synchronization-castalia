...
{
	RoutingPacket *netPkt = check_and_cast <RoutingPacket*>(pkt);
	string packetType = netPkt->getName();

	//syncPaket
	syncPacket *appPkt = check_and_cast <syncPacket*>(netPkt->decapsulate());
	...
	//Passes Timestamps
	appPkt->setRcvTime(netPkt->getRcvMacTime());
	appPkt->setTrsTime(netPkt->getTrsMacTime());
