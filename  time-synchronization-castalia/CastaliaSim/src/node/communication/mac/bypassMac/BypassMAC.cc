...
void BypassMAC::fromNetworkLayer(cPacket * pkt, int destination)
{
	...
	//
	//macFrame->setTrsMacTime(getClock());
...
void BypassMAC::fromRadioLayer(cPacket * pkt, double rssi, double lqi)
{
	...
	//
	//macPkt->setRcvMacTime(getClock());

	//trace() << "MAC TimerService offset: " << getOffset() << " ResourceManager offset: " <<resMgrModule->getOffset();
	//trace() << "MAC offset: " << macPkt->getRcvMacTime() - macPkt->getTrsMacTime();
