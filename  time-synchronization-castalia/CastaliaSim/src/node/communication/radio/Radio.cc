...
void Radio::initialize()
{
	...
	//resourceManager pointer. used by TimerService
    ...
    setResourceModule(resMgrModule);
...
void Radio::handleMessage(cMessage * msg)
{
	...
	switch (msg->getKind()) {
		...
		case WC_SIGNAL_START:{
			...
			//receive signal. Message time stamp
			rcvTime = getClock();
			trace() << "rcvTime: " << rcvTime << "  clock: " << getClock() << " radio Timer offset: " << resMgrModule->getOffset();
		...
		case WC_SIGNAL_END:{
			...
			if (endingSignal->bitErrors != ALL_ERRORS) {
				if (endingSignal->bitErrors <= maxErrorsAllowed(endingSignal->encoding)) {
					...
					//Insert Timestamp  in Mac frame
					macPkt->setRcvMacTime(rcvTime);
...
double Radio::popAndSendToWirelessChannel()
{
	...
	trsTime = getClock();
	macPkt->setTrsMacTime(trsTime);
	trace() << "trsTime: " << macPkt->getTrsMacTime() << "  clock: " << getClock();
